...
bool PluginManager::loadPlugin(const char* path) {
    ...
    if (!path)
        return false;
#ifdef _WIN32
	auto deleter = [](wchar_t* p) { free(p); };
    extern wchar_t* util_mbstowcs(const char* mbs);
    std::unique_ptr<wchar_t, decltype(deleter)> wpath;
    wpath.reset(util_mbstowcs(path));
    HMODULE h = LoadLibraryW(wpath.get());
    if (!h) {
