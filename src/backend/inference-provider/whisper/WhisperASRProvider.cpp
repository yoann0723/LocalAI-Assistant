...
Status WhisperASRProvider::initialize(const char* model_path, const Model_Params& params)
{
    ...
    ctx_wsp = whisper_init_from_file_with_params(model_path, cparams);
    if (ctx_wsp == nullptr) {
    ...
    std::span<const float> samples, std::string& text_heard)
{
    ...
    float prob0 = 0.0f;
    int64_t t_ms = 0;
    auto all_heard = ::trim(::transcribe(ctx_wsp, params, samples.data(), samples.size(), k_prompt_whisper, prob0, t_ms));
